<div>
    <flux:modal name="edit-task" variant="flyout" x-data="{ selectedTab: 'task' }" x-on:close="selectedTab = 'task'">
        @if($new)
            <flux:heading size="lg">{{ __('kanban-board::messages.new_task') }}</flux:heading>
            <div class="space-y-6 mt-6">
                ...
                />
                <flux:field>
                    <flux:label>{{ __('kanban-board::messages.label') }}</flux:label>
                ...
                <div class="flex">
                    <flux:spacer />
                    <flux:button type="submit" variant="primary" wire:click="createTask">{{ __('kanban-board::messages.save') }}</flux:button>
                </div>
            </div>
        ...
        @else
            <div >
                <flux:radio.group variant="segmented">
                    <flux:radio label="{{ __('kanban-board::messages.task') }}" x-on:click="selectedTab = 'task'" x-bind:checked="selectedTab === 'task'"/>
                    <flux:radio label="{{ __('kanban-board::messages.comment') }}" x-on:click="selectedTab = 'comment'" />
                    <flux:radio label="{{ __('kanban-board::messages.file') }}" x-on:click="selectedTab = 'file'"/>
                </flux:radio.group>
                <div  x-show="selectedTab === 'task'">
                    <div class="space-y-6 mt-6">
                        @if($task)
                                        ...
                                        @if($title)
                                            ...
                                            <div>
                                                <flux:dropdown>
                                                    ...
                                                    <flux:menu>
                                                        @if($task->isSubtask())
                                                            <flux:menu.item icon="arrows-right-left" x-on:click="$flux.modal('main-task').show()">{{ __('kanban-board::messages.change_main_task') }}</flux:menu.item>
                                                            <flux:menu.item icon="scissors" wire:click="unlinkMainTask({{ $task->id }})">{{ __('kanban-board::messages.unlink_main_task') }}</flux:menu.item>
                                                        @else
                                                            <flux:menu.item icon="arrows-right-left" x-on:click="$flux.modal('main-task').show()">{{ __('kanban-board::messages.convert_to_subtask') }}</flux:menu.item>
                                                        @endif
                                                        <flux:menu.item icon="trash" variant="danger" wire:click="DeleteSubTask({{ $task->id }})">{{ __('kanban-board::messages.delete') }}</flux:menu.item>
                                                    </flux:menu>
                                                </flux:dropdown>
                                            </div>
                                        @else
                                            <span class="text-gray-400 dark:text-gray-500 italic" wire:click="startEditing">{{ __('kanban-board::messages.click_to_edit') }}</span>
                                        @endif
                                    </div>
                                @else
                                    <div >
                            ...
                            <div>
                                <flux:text class="mt-2">#{{$task->id}} {{ __('kanban-board::messages.task_author') }} {{$task->createdUser->name}} {{ __('kanban-board::messages.created_at') }} {{$task->created_at}}</flux:text>
                            </div>
                            <flux:textarea
                                label="{{ __('kanban-board::messages.description') }}"
                                wire:model="description"
                            ...
                            />
                            <flux:input label="{{ __('kanban-board::messages.start_date') }}" placeholder="" type="date" wire:model.blur="start_date" :disabled="!$this->project->users->where('id', auth()->id())->first()"/>
                            <flux:input label="{{ __('kanban-board::messages.due_date') }}" placeholder="" type="date" wire:model.blur="due_date" :disabled="!$this->project->users->where('id', auth()->id())->first()"/>
                            <flux:input label="{{ __('kanban-board::messages.reminder') }}" placeholder="" type="date" wire:model.blur="reminder_at" :disabled="!$this->project->users->where('id', auth()->id())->first()"/>
                            <flux:select wire:model.blur="assigned_user" label="{{ __('kanban-board::messages.assignee') }}" placeholder="{{ __('kanban-board::messages.select_assignee') }}" :disabled="!$this->project->users->where('id', auth()->id())->first()">
                            ...
                            <!-- レビュワー -->
                            <flux:field>
                                <flux:label>{{ __('kanban-board::messages.reviewer') }}</flux:label>
                                <div class="flex gap-2 flex-wrap">
                                    <flux:button variant="subtle" icon="plus" size="sm" tooltip="{{ __('kanban-board::messages.add_reviewer') }}"  wire:click="openReviewerModal" :disabled="!$this->project->users->where('id', auth()->id())->first()"/>
                                ...
                                <flux:modal name="edit-reviewer" class="md:w-96">
                                    <form wire:submit.prevent="addReviewer">
                                        <div class="space-y-6">
                                            <div>
                                                <flux:heading size="lg">{{ __('kanban-board::messages.reviewer') }}</flux:heading>
                                            </div>
                                            <flux:input wire:model.live="name" placeholder="{{ __('kanban-board::messages.search_placeholder') }}"/>
                                            <div class="space-y-2">
                                            ...
                                            <div class="flex">
                                                <flux:spacer />
                                                <flux:button type="submit" variant="primary">{{ __('kanban-board::messages.add') }}</flux:button>
                                            </div>
                                        </div>
                                    </form>
                                </flux:modal>
                            ...
                            <!-- フォロワー -->
                            <flux:field>
                                <flux:label>{{ __('kanban-board::messages.follower') }}</flux:label>
                                <div class="flex gap-2 flex-wrap">
                                    <flux:button variant="subtle" icon="plus" size="sm" tooltip="{{ __('kanban-board::messages.add_follower') }}"  wire:click="openFollowerModal" :disabled="!$this->project->users->where('id', auth()->id())->first()"/>
                                ...
                                <flux:modal name="edit-follower" class="md:w-96">
                                    <form wire:submit.prevent="addFollower">
                                        <div class="space-y-6">
                                            <div>
                                                <flux:heading size="lg">{{ __('kanban-board::messages.follower') }}</flux:heading>
                                            </div>
                                            <flux:input wire:model.live="name" placeholder="{{ __('kanban-board::messages.search_placeholder') }}"/>
                                            <div class="space-y-2">
                            ...
                            <flux:field>
                                <flux:label>
                                    <flux:icon.tag />
                                    {{ __('kanban-board::messages.tag') }}
                                </flux:label>
                                <div class="flex gap-2 flex-wrap">
                                    <flux:button variant="subtle" icon="plus" size="sm" tooltip="{{ __('kanban-board::messages.add_tag') }}"  wire:click="openTagModal" :disabled="!$this->project->users->where('id', auth()->id())->first()"/>
                                    @if($task)
                                ...
                                <flux:modal name="edit-tags" class="md:w-96">
                                    <div class="space-y-6">
                                        <form wire:submit.prevent="addBadge">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg">{{ __('kanban-board::messages.add_tag') }}</flux:heading>
                                                </div>
                                                <flux:input wire:model.live="search_tag" placeholder="{{ __('kanban-board::messages.search_placeholder') }}"/>
                                                <div class="space-y-2">
                                        ...
                                        <flux:modal.trigger name="add-tag">
                                            <flux:button icon="plus" class="w-full">{{ __('kanban-board::messages.create_new_badge') }}</flux:button>
                                        </flux:modal.trigger>
                                    </div>
                                </flux:modal>
                                <flux:modal name="add-tag" class="md:w-96">
                                    <div class="space-y-6">
                                        <div>
                                            <flux:heading size="lg">{{ __('kanban-board::messages.create_new_badge') }}</flux:heading>
                                        </div>
                                        <flux:input label="{{ __('kanban-board::messages.badge_name') }}" placeholder="{{ __('kanban-board::messages.badge_name') }}" wire:model.live.debounce="tag_name"/>

                                        <flux:field>
                                            <flux:label>{{ __('kanban-board::messages.similar_tags') }}</flux:label>
                                        ...
                                        <flux:field>
                                            <flux:label>{{ __('kanban-board::messages.color') }}</flux:label>
                                            <div class="flex gap-2">
                                        ...
                                        <div class="flex">
                                            <flux:spacer />
                                            <flux:button type="submit" variant="primary" wire:click="addNewBadge">{{ __('kanban-board::messages.create') }}</flux:button>
                                        </div>
                                    </div>
                                </flux:modal>
                            ...
                            <flux:field>
                                <flux:label>
                                    ...
                                    {{ __('kanban-board::messages.subtask') }}
                                </flux:label>
                                <div class="space-y-0.5" x-data="check">
                                    ...
                                    @if($this->project->users->where('id', auth()->id())->first())
                                        <flux:input wire:model="subTitle" wire:keydown.enter="AddSubTaskItem" placeholder="{{ __('kanban-board::messages.subtask') }}{{ __('kanban-board::messages.add') }}" />
                                    @endif
                                </div>
                            ...
                            <flux:field>
                                <flux:label>
                                    ...
                                    {{ __('kanban-board::messages.checklist') }}
                                </flux:label>
                                <div class="rounded-lg bg-zinc-400/5 dark:bg-zinc-900 divide-y divide-zinc-400/5 border" x-data="check">
                                    ...
                                    <div class="flex items-center p-2 gap-2">
                                        @if($this->project->users->where('id', auth()->id())->first())
                                            ...
                                            <flux:input wire:model="checkItem" wire:keydown.enter="AddCheckItem" placeholder="{{ __('kanban-board::messages.checklist') }}{{ __('kanban-board::messages.add_item') }}" />
                                        @endif
                                    </div>
                                </div>
