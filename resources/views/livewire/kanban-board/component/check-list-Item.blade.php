<div class="flex items-center p-2 gap-2">
    ...
    <div class="w-full flex flex-row items-center gap-2 group/item">
        ...
        @elseif(!$editing)
            <div wire:click="startEditing" class="w-full group/content">
                @if($content)
                    @if($item->is_completed)
            ...
            <flux:button size="xs" icon="trash" variant="ghost" inset class="opacity-0 group-hover/item:opacity-100" wire:click="$parent.DeleteCheckItem({{ $item->id }})"/>
        @else
            <div >
